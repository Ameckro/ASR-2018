...
Al arrancar el sistema, se ejecuta la BIOS, cuyo objetivo es buscar el cargador del SO. La bios es parametizable. Por ejemplo, un parametro que podemos cambirar, es el orden de busqueda del cargador. En un servidor, la BIOS tiene que estar protegida con una contraseña. Si encuentra entre el orden establecido de los dispositvos conectados el cargador lo ejecuta. El cargador (o bootloader), inicia el sistema operativo y ejecuta el proceso ```vinit```. Dicho proceso es el encargado de poner en marcha todos los servicios del sistema. En el fichero ```/etc/inittab``` se indica que modo de arranque tiene el sistem(runlevel): 0 -> halt, 1 -> single user(root), 2..5-> multi user, 6 reboot. Cada uno de esos modos (niveles), en el directorio ```/etc``` hay unos directorios ```rc1, rc2, ... rc6``` y en cada uno de los directorios, hay una serie de ficheros que empiezan por ```K``` (se ejecutan al parar la maquina (KILL)) y otros que empiezan con ```S``` (se ejecutan al iniciar la maquina (START)). Cada uno de esos ficheros ademas, tien un numero indicando el orden de ejecucion. Ese numero se establece por las dependencias internas de las aplicaciones. El directrorio ```/etc/init.d``` se guarda todos los scripts de las aplicaciones para arrancarlas/parardas.
El script ```/etc/init.d/rc``` es el encargado de gestionar todo lo anterior

La característica central de ```systemd```  es el (“Socket activation”), que permite iniciar todos los servicios (procesos servidores) en el arranque del sistema de forma concurrente, sin tener que especificar de forma explícita las dependencias entre los servicios (como si ocurre en “sysvinit”). Aparecen las ```unidades```, donde se especifica las dependencias, y el orden de ejecucion. Los ficheros ```Target``` son una agrupacion de unidades con el objetivo de lanzarlas de forma conjunta. Al igual que en ```sistemv``` existia el fichero ```etc/inittab```, en ```sistemd``` es el fichero ```default.target```. Los servicios se configuran con los ficheros unit con el mismo nombre. con el comando ```systemctl``` informa del estado de todas las unidades del sistema.
